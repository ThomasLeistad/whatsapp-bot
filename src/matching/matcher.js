...
async function buscarEnSocios(datosComprobante) {
  ...
  // Exacto por alias (más confiable)
  if (alias) {
    const rows = await query(
      `SELECT sa.*, dp.id as pedido_id, dp.nombre, dp.apellido, dp.email,
              dp.pagado, dp.estado, dp.pedidoID
       FROM socios_alias sa
       LEFT JOIN datospedidos dp ON sa.pedido_id = dp.id
       WHERE LOWER(sa.alias_mp) = LOWER(?) AND sa.confirmado = 1`,
      [alias]
    );
    if (rows.length > 0) return { match: rows[0], score: 1.0, tipo: 'alias_exacto', automatico: true };
  }

  // Exacto por CBU
  if (cbu) {
    const rows = await query(
      `SELECT sa.*, dp.id as pedido_id, dp.nombre, dp.apellido, dp.email, dp.pedidoID
       FROM socios_alias sa
       LEFT JOIN datospedidos dp ON sa.pedido_id = dp.id
       WHERE sa.cbu = ? AND sa.confirmado = 1`,
      [cbu]
    );
    if (rows.length > 0) return { match: rows[0], score: 1.0, tipo: 'cbu_exacto', automatico: true };
  }

  // Fuzzy por nombre en socios conocidos
  if (nombre) {
    const socios = await query(
      `SELECT sa.*, dp.id as pedido_id, dp.nombre as ped_nombre, dp.apellido as ped_apellido, dp.pedidoID
       FROM socios_alias sa
       LEFT JOIN datospedidos dp ON sa.pedido_id = dp.id
...
async function buscarEnPedidos(datosComprobante) {
  ...
  if (monto) {
    sqlWhere += ` AND ABS(COALESCE(dp.total, 0) - ?) / GREATEST(COALESCE(dp.total, 1), 1) < 0.05`;
    params.push(monto);
  }
  ...
  if (!pedidos.length || !nombre) return [];
  ...
  return fuse.search(nombre).slice(0, 3).map(r => ({
    pedido: r.item,
    score:  1 - r.score,
  }));
}
...
async function buscarMatch(datosComprobante) {
  ...
  if (matchSocio) {
    logger.info(`Match automático (score ${matchSocio.score.toFixed(2)}) via ${matchSocio.tipo}`);
    return {
      automatico: true,
  ...
  return { automatico: false, tipo: 'busqueda_pedidos', candidatos };
}
...
async function aprenderMatch(pedidoId, datosComprobante) {
  ...
  const existente = await query('SELECT id FROM socios_alias WHERE pedido_id = ?', [pedidoId]);
